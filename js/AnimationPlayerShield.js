class AnimationPlayerShield extends Animation{
	
	//a = x-coordinate, b = y-coordinate, c = length of display, d = height of display
	
	constructor(p, a, b, c, d) {
		super("PlayerShield",a,b,c,d);
		this.file = "PlayerShield.png";
		this.tic = 0;
		this.maxTics = 25;
		this.aniWidth = 960/5;
	...
	paintAnimation(g) {
		...
		g.drawImage(this.gfx, (this.frame%this.aniRowNumber)*this.aniWidth, Math.floor(this.frame/this.aniRowNumber)*this.aniHeight, this.aniWidth, this.aniHeight, 
		                    this.data[0]-50, this.data[1]-50, this.data[2]+100, this.data[3]+100);
		this.tic++;
		this.frame = this.tic;
		if(this.tic == 25) this.play.makeEquationShield();
